...
def extract_frames_from_video(
    video_path: str, number_of_frames_to_extract: int, out_dir: str
):
    cap = cv2.VideoCapture(video_path)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

    frames_to_extract = [
        math.floor(i * total_frames / number_of_frames_to_extract)
        for i in range(number_of_frames_to_extract)
    ]
        ...
        with tqdm(total=len(videos_paths)) as pbar:
            for v in p.imap_unordered(
                partial(
                    ...
                    number_of_frames_to_extract=NUM_FRAMES_TO_EXTRACT,
                    out_dir=OUT_DIR,
                ),
                videos_paths,
                chunksize=4,
